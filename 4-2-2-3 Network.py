...
for itr in range(iterations):
    ...
    quickprop_eta3 = np.where(dW3_change != 0, W3_delta / (dW3_change + min_update), 1.0)
    quickprop_eta3 = np.minimum(quickprop_eta3, mu)

    # Update weights with Quickprop
    W1 -= quickprop_eta * dW1_change
    W2 -= quickprop_eta2 * dW2_change
    W3 -= quickprop_eta3 * dW3_change
